...
void set_value(GraphicsOption option, int value) {
    ...
    case GraphicsOption::ShadowResolution:
        ...
        break;
    case GraphicsOption::Resampler: {
        const auto sampler = static_cast<Resampler>(std::clamp(
            value, static_cast<int>(Resampler::Bilinear), static_cast<int>(Resampler::Area)));
        getSettings().game.resampler.setValue(sampler);
        aurora_set_resampler(sampler == Resampler::Area ? SAMPLER_AREA : SAMPLER_BILINEAR);
        break;
    }
    case GraphicsOption::BloomMode:
...
Rml::String format_graphics_setting_value(GraphicsOption option, int value) {
    ...
    case GraphicsOption::Resampler:
        return value == static_cast<int>(Resampler::Area) ? "Area" : "Bilinear";
    case GraphicsOption::BloomMode:
