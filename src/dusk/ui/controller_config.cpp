...
Rml::String current_controller_name(int port) {
    ...
    if (port == 0 && dusk::IsMobile && getSettings().game.enableTouchControls.getValue() &&
    ...
    case Page::Controller: {
                ...
                .isSelected =
                    [port] {
                        return PADGetIndexForPort(port) < 0 && !keyboard_active(port) &&
                               !(port == 0 && dusk::IsMobile && getSettings().game.enableTouchControls.getValue());
                    },
            })
        ...
        if (dusk::IsMobile) {
            pane.add_button({
                                .text = "Touch",
                                .isSelected = [port] {
                                    return port == 0 && getSettings().game.enableTouchControls.getValue() &&
                                           PADGetIndexForPort(port) < 0 && !keyboard_active(port);
                                },
                            })
                .on_pressed([this, port] {
                    ...
                    PADClearPort(port);
                    PADSetKeyboardActive(static_cast<u32>(port), FALSE);
                    getSettings().game.enableTouchControls.setValue(true);
                    config::Save();
                    PADSerializeMappings();
                });
        }
